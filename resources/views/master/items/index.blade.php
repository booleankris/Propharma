            ...
            <div class="flex flex-col lg:flex-row gap-4">
                        ...
                        <div class="w-56">
                            <select id="filter_pharmacy" class="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-xs focus:ring-2 focus:ring-blue-200">
                                ...
                                @foreach($pharmacies as $p)
                                    <option value="{{ $p->id }}" {{ getActivePharmacyId() == $p->id ? 'selected' : '' }}>{{ $p->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div class="bg-white p-6 rounded-2xl shadow-md w-full md:w-[35%] mx-auto">
                    <form id="compositionForm" action="{{ route('items.store') }}" method="POST" class="space-y-3">
                        ...
                        <div>
                            <label class="block text-[14px] font-semibold text-gray-800 mb-1">Cabang</label>
    ...
    <script>
        ...
        $(function() {
            ...
            $('#table-data tbody').on('click', 'tr', function() {
                ...
                $('#pharmacy_id').val(selectedData.pharmacy_id || '');
            });
            // BACK
            $('#back').click(function() {
