...
@section('style')
    ...
    <style>
        ...
        .select2-selection__choice { background:#e5e7eb !important; border-radius:6px !important; padding:4px 8px !important; font-size:13px; }

        /* ── Table row states ── */
        #table-data tbody tr { cursor: pointer; transition: background .15s; }
        #table-data tbody tr:hover  { background: #eff6ff; }
        #table-data tbody tr.active-row { background: #dbeafe !important; }
        ...
        #formModeBar.edit  { display: flex; background: #fef9c3; color: #92400e; border: 1px solid #fde68a; }
        #formModeBar.add   { display: flex; background: #dcfce7; color: #166534; border: 1px solid #86efac; }

        /* ── Status badges ── */
        .badge-active   { padding:2px 10px; background:#dcfce7; color:#166534; border-radius:999px; font-size:12px; }
        .badge-inactive { padding:2px 10px; background:#fee2e2; color:#991b1b; border-radius:999px; font-size:12px; }
        ...
        .creditor-pill:hover { background: #2563eb; }
        .creditor-pill button { background: none; border: none; color: #fff; font-size: 16px; line-height: 1; cursor: pointer; padding: 0; }

        /* ── Double-click hint ── */
        .dbl-hint { font-size: 11px; color: #9ca3af; margin-top: 4px; display: flex; align-items: center; gap: 4px; }
    </style>
@endsection
                ...
                <form id="medicineForm" action="{{ route('medicines.store') }}" method="POST" class="space-y-2">
                    ...
                    <div class="flex gap-1">
                        <div class="w-full">
                            ...
                            <select id="packaging" name="packaging" class="select2 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-[13px]">
                        ...
                        <div class="w-full">
                            ...
                            <select id="unit" name="unit" class="select2 w-full rounded-lg border border-gray-300 px-4 py-2.5 text-[13px]">
...
function setSelect2Value(selector, id, text) {
    const $s = $(selector);
    if (!id) { $s.val(null).trigger('change'); return; }
...
function populateForm(row) {
    ...
    ['code','name','barcode','content','dosage',
     'minimal_stock','stock','preparations'].forEach(key => {
    ...
    // generic checkbox
    document.getElementById('generic_check').checked = !!row.generic;
...
const NAV_ORDER = [
    { id: 'medicine_category_id', type: 'select2ajax' },
    { id: 'composition_id',       type: 'select2ajax' },
    { id: 'factory_id',           type: 'select2ajax' },
    { id: 'name',                 type: 'input' },
    { id: 'barcode',              type: 'input' },
    { id: 'packaging',            type: 'select2' },
    { id: 'unit',                 type: 'select2' },
    { id: 'content',              type: 'input' },
    { id: 'dosage',               type: 'input' },
    { id: 'preparations',         type: 'input' },
    { id: 'pharmacy_net_price',   type: 'input' },
    { id: 'het_price',            type: 'input' },
    { id: 'minimal_stock',        type: 'input' },
    { id: 'searchInput',          type: 'creditor' },
    { id: 'location',             type: 'select2ajax' },
    { id: 'items',                type: 'select2ajax' },
    { id: 'type',                 type: 'select2' },
];
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    elHet.addEventListener('input', function() { formatField(this); });
    ...
    $('#packaging').select2({ placeholder: 'Pilih Kemasan...',  allowClear: true });
    $('#unit').select2(     { placeholder: 'Pilih Satuan...',   allowClear: true });
    $('#type').select2(     { placeholder: 'Pilih Tipe...',     allowClear: true });
    ...
    document.getElementById('deleteData').addEventListener('click', () => {
        const id = document.getElementById('medicine_id').value;
        if (!id) {
            iziToast.warning({ title: 'Perhatian', message: 'Pilih data obat terlebih dahulu.' });
            return;
        }
        swal({
            title: 'Hapus Obat?',
            ...
            axios.delete('/medicines/' + id)
                .then(({ data }) => {
                    iziToast.success({ title: 'Dihapus', message: data.message });
                    resetForm();
                    tableData.ajax.reload(null, false);
                });
        });
