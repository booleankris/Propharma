...
class SalesReturExport implements FromArray, WithStyles, WithColumnWidths, WithTitle
{
    ...
    public function array(): array
    {
        ...
        $rows[] = ['LAPORAN RETUR PENJUALAN (KASIR)'];
        $rows[] = ['Periode: ' . $this->startDate->format('d/m/Y') . ' s/d ' . $this->endDate->format('d/m/Y')];
        $rows[] = [''];

        // Table headers
        $rows[] = ['NO', 'KODE TRANSAKSI', 'TANGGAL', 'NAMA OBAT', 'PASIEN / PELANGGAN', 'QTY RETUR', 'TOTAL (RP)'];

        $logs = ItemsLog::with(['medicines', 'batches'])
            ->where('status', 3) // 3 = Retur Penjualan
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        if ($this->lastRow >= 6) {
            ...
            $sheet->getStyle("A{$this->lastRow}:G{$this->lastRow}")->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB('FEF3C7');
        }
    }
