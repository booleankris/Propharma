        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 mb-6 border-b border-slate-200">
            <div>
                <div class="flex items-center gap-2 text-xs font-medium text-slate-400 mb-1">
                    ...
                    <span class="text-sky-600 font-semibold">Buat Transfer Stok</span>
                </div>
                <h1 class="text-xl font-bold  tracking-tight">Transfer Stok Obat</h1>
            </div>

            <div class="flex items-center gap-3">
                <div class="bg-[#2563eb] border border-slate-200/80 px-3 py-1.5 rounded-lg flex items-center gap-2">
                    <span class="text-[11px] font-semibold text-white uppercase tracking-wider">Kode Mutasi</span>
                    <span class="text-xs font-bold text-[#ffff00]">{{ $code }}</span>
                </div>
                <div class="bg-[#2563eb] border border-slate-200/80 px-3 py-1.5 rounded-lg flex items-center gap-2">
                    <span class="text-[11px] font-semibold text-[#fff] uppercase tracking-wider">Tanggal</span>
                    <span class="text-xs font-semibold text-[#ffff00]">{{ $now }}</span>
                </div>
            </div>
        </div>
                ...
                <div class="relative w-full" id="searchWrapper">
                    <div class="relative">
                        ...
                        <input type="text" id="searchInput"
                            class="w-full pl-10 pr-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-lg text-sm text-slate-800 placeholder-slate-400 focus:border-sky-500 focus:bg-white focus:ring-4 focus:ring-sky-500/10 outline-none transition"
                ...
                <div id="stagingCard" class="hidden mt-4 p-4 rounded-xl bg-sky-50/40 border border-sky-200/60 space-y-4">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="number" id="stageQty"
                                    class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm font-mono text-slate-800 outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-500/10"
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-2">
                                <select id="stageEtalase"
                                    class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm text-slate-800 outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-500/10">
                                    <option value="">— Pilih etalase —</option>
                                </select>
                                <button type="button" onclick="openEtalaseModal('create')" title="Tambah etalase"
                                    class="shrink-0 w-9 h-9 rounded-lg border border-slate-200 bg-white text-slate-500 hover:bg-slate-50 hover:text-sky-600 flex items-center justify-center transition">
                    ...
                    <div class="flex items-center gap-2 pt-1">
                        <button type="button" id="addItemBtn" onclick="addItemToCart()" disabled
                            class="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2 bg-sky-600 hover:bg-sky-700 disabled:bg-slate-200 disabled:text-slate-400 text-white font-medium text-xs rounded-lg transition shadow-sm">
                            + Tambah ke Daftar
                        </button>
                        <button type="button" onclick="clearStaging()"
                            class="px-4 py-2 bg-white hover:bg-slate-100 border border-slate-200 text-slate-600 font-medium text-xs rounded-lg transition">
                            Batal
                        </button>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 sm:p-6">
                ...
                <div class="flex items-center justify-end gap-3 mt-6 pt-4 border-t border-slate-100">
                    <button type="button" onclick="window.location.href='{{ route('home') }}'"
                        class="px-4 py-2 bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 font-medium text-xs rounded-lg transition">
                        Batal
                    </button>
                    <button type="button" id="submitBtn" onclick="submitTransfer()" disabled
                        class="inline-flex items-center gap-2 px-5 py-2 bg-sky-600 hover:bg-sky-700 disabled:bg-slate-200 disabled:text-slate-400 text-white font-semibold text-xs rounded-lg shadow-sm transition">
                        Simpan Transfer
                    </button>
                </div>
            </div>
            ...
            fetch(`{{ route('search.getbatches') }}?search=${encodeURIComponent(keyword)}&page=${page}`)
                ...
                .then(res => {
                    ...
                    res.data.forEach((item, i) => {
                        ...
                        row.className =
                            'dropdown-row grid grid-cols-[32px_1fr_1fr_80px] px-4 py-2.5 text-xs cursor-pointer hover:bg-sky-50 transition items-center border-b border-slate-50';
