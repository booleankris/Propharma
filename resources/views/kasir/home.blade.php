    ...
    <div class="kasir-home px-4 md:px-8 py-2 w-full space-y-5">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm">
            ...
            <div class="flex items-center gap-2.5 self-start md:self-auto">
                <div class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-slate-100/80 border border-slate-200 text-slate-700 text-xs font-semibold">
                    <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                    <span id="live-clock">{{ date('d M Y') }}</span>
                </div>
                <div class="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-blue-50 border border-blue-200/70 text-blue-700 text-xs font-bold">
        ...
        {{-- Digital Role Banner --}}
        @if (auth()->user()->hasRole('Digital'))
            <div class="p-4 rounded-2xl bg-gradient-to-r from-indigo-500/10 via-purple-500/10 to-blue-500/10 border border-indigo-200/60 flex items-center justify-between gap-4 shadow-sm">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-600 to-purple-600 text-white flex items-center justify-center shadow-md shadow-indigo-500/20 shrink-0">
        ...
        {{-- HO Banner --}}
        @role('HO')
        <div class="p-5 rounded-2xl bg-gradient-to-r from-indigo-950 via-slate-900 to-blue-950 text-white shadow-md border border-indigo-900/60 flex flex-col sm:flex-row items-center justify-between gap-4 relative overflow-hidden">
            <div class="absolute -right-10 -bottom-10 w-44 h-44 bg-indigo-500/20 rounded-full blur-2xl pointer-events-none"></div>
            <div class="flex items-center gap-4 relative z-10">
                <div class="w-12 h-12 rounded-2xl bg-white/10 backdrop-blur-md flex items-center justify-center border border-white/20 text-indigo-200 shrink-0">
                ...
                <div>
                    <div class="flex items-center gap-2">
                        <h3 class="text-base font-bold tracking-tight text-white">Executive Analytics &amp; Monitoring HO</h3>
                    ...
                    <p class="text-xs text-indigo-200/90 mt-0.5">Monitoring omzet penjualan, belanja pembelian, retur, tren bulanan &amp; grafik jenis transaksi</p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 {{ isOnlineRole() ? 'lg:grid-cols-3' : 'lg:grid-cols-4' }} gap-4">
            ...
            <div class="stat-card-glow relative overflow-hidden rounded-2xl border border-blue-200/70 bg-gradient-to-br from-blue-600 via-blue-600 to-indigo-700 p-4 sm:p-5 text-white shadow-md shadow-blue-500/10">
                <div class="flex items-start justify-between">
                    <div>
                        <span class="text-[11px] font-bold uppercase tracking-wider text-blue-100 flex items-center gap-1.5">
                            <span class="w-1.5 h-1.5 rounded-full bg-blue-300"></span>
                            {{ isOnlineRole() ? 'Penjualan Hari Ini' : 'Penjualan Hari Ini' }}
                        </span>
                        <h3 class="text-xl sm:text-2xl font-extrabold text-white mt-1 tracking-tight truncate">{{ $today_sales_rp }}</h3>
                    </div>
                    <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-white/15 backdrop-blur border border-white/20">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                ...
                <div class="mt-3 flex items-center justify-between pt-2 border-t border-white/15 text-[11px] text-blue-100">
                    <span>Transaksi hari ini:</span>
                    <span class="font-bold bg-white/20 px-2 py-0.5 rounded-full text-white">{{ $today_qty_sales ?? 0 }} Struk</span>
                </div>
            </div>

            {{-- Card 2: Penjualan Bulan Ini --}}
            <div class="stat-card-glow relative overflow-hidden rounded-2xl border border-indigo-200/70 bg-white p-4 sm:p-5 shadow-sm hover:border-indigo-300">
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <h3 class="text-xl sm:text-2xl font-extrabold text-slate-800 mt-1 tracking-tight truncate">{{ $month_sales_rp }}</h3>
                    </div>
                    <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600 border border-indigo-100">
                ...
                <div class="mt-3 flex items-center justify-between pt-2 border-t border-slate-100 text-[11px] text-slate-500">
            ...
            @if (isOnlineRole())
                ...
                <div class="stat-card-glow relative overflow-hidden rounded-2xl border border-emerald-200/70 bg-white p-4 sm:p-5 shadow-sm hover:border-emerald-300">
                    <div class="flex items-start justify-between">
                        <div>
                            <span class="text-[11px] font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
                            ...
                            <h3 class="text-xl sm:text-2xl font-extrabold text-slate-800 mt-1 tracking-tight truncate">{{ number_format($qty_sales ?? 0, 0, ',', '.') }} <span class="text-sm font-semibold text-slate-400">Struk</span></h3>
                        </div>
            ...
            @else
                ...
                <div class="stat-card-glow relative overflow-hidden rounded-2xl border border-emerald-200/70 bg-white p-4 sm:p-5 shadow-sm hover:border-emerald-300">
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <h3 class="text-xl sm:text-2xl font-extrabold text-slate-800 mt-1 tracking-tight truncate">{{ $total_orders_rp }}</h3>
                        </div>
                        <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-emerald-50 text-emerald-600 border border-emerald-100">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                                <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                ...
                <div class="stat-card-glow relative overflow-hidden rounded-2xl border border-rose-200/70 bg-white p-4 sm:p-5 shadow-sm hover:border-rose-300">
                    <div class="flex items-start justify-between">
                        <div>
                            <span class="text-[11px] font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
                            ...
                            <h3 class="text-xl sm:text-2xl font-extrabold text-slate-800 mt-1 tracking-tight truncate">{{ $total_reject_rp }}</h3>
                        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 p-5 md:p-6 shadow-sm space-y-6">
            ...
            <div>
                <div class="flex items-center justify-between mb-3 px-1">
                    <div class="flex items-center gap-2">
                        <span class="w-2.5 h-2.5 rounded-full {{ isWarehousePharmacy() ? 'bg-violet-500' : 'bg-blue-500' }} animate-pulse"></span>
                ...
                <div class="grid grid-cols-1 {{ isOnlineRole() ? 'max-w-xl' : 'md:grid-cols-2' }} gap-4">
                    @if (isWarehousePharmacy())
                        ...
                        <a onclick="openModal('transfersModal')" id="btn-action-primary"
                            ...
                            <div class="flex items-center gap-4 relative z-10">
                                <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-white/15 backdrop-blur-md border border-white/20 shadow-inner transition-transform duration-300 group-hover:scale-105">
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="kbd-badge text-[10px] font-extrabold px-2 py-0.5 uppercase tracking-wider text-white">F1</span>
                                    </div>
                                    <span class="text-xs text-violet-100/90 font-medium block mt-0.5">Distribusi &amp; transfer barang ke apotek cabang</span>
                                </div>
                            </div>
                            <div class="relative z-10 flex items-center justify-center w-8 h-8 rounded-full bg-white/10 text-white/80 group-hover:bg-white/20 group-hover:text-white transition-all shrink-0 ml-2">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 transition-transform duration-200 group-hover:translate-x-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                                    <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                            ...
                            <div class="flex items-center gap-4 relative z-10">
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="kbd-badge text-[10px] font-extrabold px-2 py-0.5 uppercase tracking-wider text-white">F2</span>
                                    </div>
                                    <span class="text-xs text-emerald-100/90 font-medium block mt-0.5">Input faktur supplier, SP &amp; verifikasi penerimaan</span>
                                </div>
                            </div>
                            <div class="relative z-10 flex items-center justify-center w-8 h-8 rounded-full bg-white/10 text-white/80 group-hover:bg-white/20 group-hover:text-white transition-all shrink-0 ml-2">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 transition-transform duration-200 group-hover:translate-x-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    @elseif (isOnlineRole())
                        ...
                        <a href="{{ url('transaction/upds') }}" id="btn-action-primary"
                            ...
                            <div class="flex items-center gap-4 relative z-10">
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        <span class="text-base font-bold tracking-tight">{{ auth()->user()->hasRole('Digital') ? 'Entri Penjualan Digital' : 'Penjualan Kasir' }}</span>
                                        <span class="kbd-badge text-[10px] font-extrabold px-2 py-0.5 uppercase tracking-wider text-white">F1</span>
                                    </div>
                                    <span class="text-xs text-white/90 font-medium block mt-0.5">Catat transaksi penjualan pesanan {{ getOnlineChannelName() }}</span>
                                </div>
                            </div>
                            <div class="relative z-10 flex items-center justify-center w-8 h-8 rounded-full bg-white/10 text-white/80 group-hover:bg-white/20 group-hover:text-white transition-all shrink-0 ml-2">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 transition-transform duration-200 group-hover:translate-x-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                            ...
                            <div class="flex items-center gap-4 relative z-10">
                                <div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-white/15 backdrop-blur-md border border-white/20 shadow-inner transition-transform duration-300 group-hover:scale-105">
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                        ...
                                        <path d="M4 14a2 2 0 0 1 2 -2h12a2 2 0 0 1 2 2v4a2 2 0 0 1 -2 2h-12a2 2 0 0 1 -2 -2z" />
                                        <path d="M8 15h.01" />
                                        <path d="M12 15h.01" />
                                        <path d="M16 15h.01" />
                                        <path d="M8 18h.01" />
                                ...
                                <div>
                                    <div class="flex items-center gap-2">
                                        <span class="text-base font-bold tracking-tight">Transaksi Kasir (POS)</span>
                                        <span class="kbd-badge text-[10px] font-extrabold px-2 py-0.5 uppercase tracking-wider text-white">F1</span>
                                    </div>
                                    <span class="text-xs text-blue-100/90 font-medium block mt-0.5">Buka kasir &amp; catat penjualan obat / resep baru</span>
                                </div>
                            </div>
                            <div class="relative z-10 flex items-center justify-center w-8 h-8 rounded-full bg-white/10 text-white/80 group-hover:bg-white/20 group-hover:text-white transition-all shrink-0 ml-2">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 transition-transform duration-200 group-hover:translate-x-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <a href="{{ route('receiving.index') }}" id="btn-action-secondary"
                            ...
                            <div class="flex items-center gap-4 relative z-10">
                                ...
                                <div>
                                    ...
                                    <span class="text-xs text-emerald-100/90 font-medium block mt-0.5">Kelola pesanan (SP), BPBA &amp; faktur masuk</span>
                                </div>
                            </div>
                            <div class="relative z-10 flex items-center justify-center w-8 h-8 rounded-full bg-white/10 text-white/80 group-hover:bg-white/20 group-hover:text-white transition-all shrink-0 ml-2">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 transition-transform duration-200 group-hover:translate-x-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div>
                <h2 class="text-xs font-bold uppercase tracking-widest text-slate-400 mb-3.5 px-1 flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round">
                        <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 {{ isWarehousePharmacy() ? 'lg:grid-cols-4' : (isOnlineRole() ? 'max-w-xl' : 'lg:grid-cols-5') }} gap-3">
                    @if (isWarehousePharmacy())
                        {{-- Laporan Pembelian --}}
                        <a onclick="openModal('orderReportModal')"
                            ...
                            <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-orange-100/80 text-orange-600 transition-transform duration-200 group-hover:scale-110">
                            ...
                            <span class="text-xs font-bold text-slate-700 group-hover:text-orange-600">Laporan Pembelian</span>
                        </a>
                        ...
                        <a onclick="openModal('receivingModal')"
                            ...
                            <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-violet-100/80 text-violet-600 transition-transform duration-200 group-hover:scale-110">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                                    <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                    <path d="M6 19m-2 0a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" />
                                    <path d="M17 19m-2 0a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" />
                            ...
                            <span class="text-xs font-bold text-slate-700 group-hover:text-violet-600">Data Pembelian</span>
                        </a>
                        ...
                        <a onclick="openModal('logModal')"
                            ...
                            <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-amber-100/80 text-amber-600 transition-transform duration-200 group-hover:scale-110">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                                    <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                    <path d="M21 16v-8a2 2 0 0 0 -1 -1.73l-7 -4a2 2 0 0 0 -2 0l-7 4a2 2 0 0 0 -1 1.73v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7 -4a2 2 0 0 0 1 -1.73z" />
                        ...
                        <a onclick="openModal('transfersModal')"
                            ...
                            <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-cyan-100/80 text-cyan-600 transition-transform duration-200 group-hover:scale-110">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                                    <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                    ...
                    @elseif (isOnlineRole())
                        ...
                        <a onclick="openModal('reportModal')"
                            ...
                            <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-rose-100/80 text-rose-600 transition-transform duration-200 group-hover:scale-110">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                                    <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                    <path d="M14 3v4a1 1 0 0 0 1 1h4" />
                                    <path d="M17 21h-10a2 2 0 0 1 -2 -2v-14a2 2 0 0 1 2 -2h7l5 5v11a2 2 0 0 1 -2 2z" />
                            ...
                            <span class="text-xs font-bold text-slate-700 group-hover:text-rose-600">Laporan Penjualan</span>
                        </a>
                    @else
                        {{-- Data Penjualan --}}
                        <a onclick="openModal('salesModal')"
                        ...
                        <a onclick="openModal('orderReportModal')"
                            ...
                            <span class="text-xs font-bold text-slate-700 group-hover:text-orange-600">Lap. Pembelian</span>
                        </a>
                        ...
                        <a href="{{ route('pareto.index') }}"
                            ...
                            <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-pink-100/80 text-pink-600 transition-transform duration-200 group-hover:scale-110">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                                    <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                                    <path d="M4 19l16 0" />
            ...
            @if (!isWarehousePharmacy() && !isOnlineRole())
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 {{ auth()->user()->hasRole('HO') ? 'lg:grid-cols-5' : 'lg:grid-cols-4' }} gap-3">
                        ...
                        <a onclick="openModal('masterModal')"
                            ...
                            <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-teal-100/80 text-teal-600 transition-transform duration-200 group-hover:scale-110">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                                    <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                        ...
                        <a href="{{ url('invoices/') }}"
                            ...
                            <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-blue-100/80 text-blue-600 transition-transform duration-200 group-hover:scale-110">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                                    <path stroke="none" d="M0 0h24v24H0z" fill="none" />
        ...
        <div class="rounded-2xl border border-slate-200/80 bg-white shadow-sm overflow-hidden">
            <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-slate-50/50">
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        ...
                        <p class="text-[11px] text-slate-400">Monitoring masa berlaku obat untuk pencegahan kerugian stok</p>
                    </div>
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left">
                    <thead>
                        <tr class="text-[11px] uppercase tracking-wider text-slate-500 bg-slate-50/40 border-b border-slate-100">
                        ...
                        @forelse ($nearExpiry as $item)
                            <tr class="hover:bg-slate-50/70 transition-colors">
                                <td class="px-5 py-3.5">
                                    ...
                                    <div class="text-[11px] text-slate-400 font-mono">{{ $item->medicines->code ?? '-' }}</div>
                                </td>
                                ...
                                <td class="px-5 py-3.5 text-right font-bold text-slate-800">{{ number_format($item->stock, 0, ',', '.') }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-5 py-10 text-center text-sm text-slate-400">
                                    <div class="flex flex-col items-center justify-center gap-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 text-slate-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                                            <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
