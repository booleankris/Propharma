        ...
        <div class="mx-auto space-y-6">
            ...
            <div class="flex items-center justify-between bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <div>
                    ...
                    @php
                        $currentOrderModel = isset($order_id) ? \App\Models\Order::with('order_items.incomingMovement.sourceItem.orders')->find($order_id) : null;
                        $hasMergedItems = $currentOrderModel && $currentOrderModel->order_items->contains(fn($item) => $item->incomingMovement !== null);
                    @endphp
                    @if(isset($order_id) && ($currentOrderModel?->is_consolidation || $hasMergedItems))
                        <div class="mt-3 flex flex-wrap items-center justify-between gap-3 p-3 bg-amber-50 border border-amber-200 rounded-xl">
                            <div class="text-xs text-amber-800">
                                @if($currentOrderModel?->is_consolidation)
            ...
            <div class="flex flex-wrap py-3 px-2 items-center justify-between gap-3 border-b border-gray-100 pb-4">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <a href="{{ route('orders.comparison', $ordersid ?? $id ?? request()->route('id')) }}" 
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" width="16" height="16" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                            <path d="M16 3l0 18" />
    ...
    <script>
        ...
        async function cancelConsolidation(orderId) {
            const result = await Swal.fire({
                ...
                text: 'Kuantitas pesanan yang digabungkan akan dikembalikan ke BPBA asal.',
                icon: 'warning',
                showCancelButton: true,
